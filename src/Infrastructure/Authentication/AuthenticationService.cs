...
namespace Blogpost.Infrastructure.Authentication
{
    public class AuthenticationService : IAuthenticationService
    {
            ...
            where TAuthenticatorParameter : AuthenticatorParameter
        {
            ...
            var result = await authenticator.Authenticate(parameter);
            if (!result.Succeeded) return AuthenticationResult.Failure(result.Errors);

            var applicationUser = await _applicationDbContext.Users
            ...
            var accessToken = _jwtFactory.Generate(applicationUser.Id);
            var refreshToken = GenerateRefreshToken(parameter.IpAddress);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!ValidateRefreshToken(token, refreshToken, out var failure)) return failure;
